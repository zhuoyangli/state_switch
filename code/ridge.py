...
import numpy as np
import logging
from utils import mult_diag, counter, pearsonr
import random
...
def ridge_cv(x, y, alphas, nfolds=5, nboots=1, chunklen=40, nchunks=20, singcutoff=1e-10, single_alpha=True):
    """Cross-validated ridge regression with bootstrapping.
    ...
    permuted_idx = np.random.permutation(y.shape[0])
    test_idx = permuted_idx[:y.shape[0] // nfolds]
    train_idx = permuted_idx[y.shape[0] // nfolds:]
    delRstim = x[train_idx, :]
    delPstim = x[test_idx, :]
    delRresp = y[train_idx, :]
    delPresp = y[test_idx, :]
    wt, corr, alphas, bscorrs, valinds = bootstrap_ridge(delRstim, delRresp, delPstim, delPresp, alphas, nboots, chunklen, nchunks, singcutoff, single_alpha)
    corr_cv = pearsonr(np.dot(delPstim, wt), delPresp)
    # corr_cv_all[ifold] = corr_cv
    return wt, corr, alphas, bscorrs, valinds, corr_cv
