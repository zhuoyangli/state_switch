...
expName = 'ses-002_task-stories'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/gio/projects/SwitchCode/ses-002_task-stories/ses-002_task-stories.py',
...
def setupDevices(expInfo, thisExp, win):
    ...
    # create speaker 'story_audio'
    deviceManager.addDevice(
        deviceName='story_audio',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "present_story_num" ---
    story_num_txt = visual.TextStim(win=win, name='story_num_txt',
        text='',
        font='Arial',
    ...
    story_audio = sound.Sound(
        'A', 
        secs=-1, 
        stereo=True, 
        hamming=True, 
        sampleRate=44100,
        speaker='story_audio',    name='story_audio'
    )
    story_audio.setVolume(1.0)
    ...
    for thisStory_loop in story_loop:
        ...
        for thisGo_back in go_back:
            ...
            # --- Prepare to start Routine "present_story_num" ---
            continueRoutine = True
            # update component parameters for each repeat
            thisExp.addData('present_story_num.started', globalClock.getTime(format='float'))
            story_num_txt.setText('Story ' + str(story_loop.thisN + 1))
            ...
            # keep track of which components have finished
            present_story_numComponents = [story_num_txt, key_resp_story_start]
            for thisComponent in present_story_numComponents:
                thisComponent.tStart = None
                thisComponent.tStop = None
                thisComponent.tStartRefresh = None
                thisComponent.tStopRefresh = None
            ...
            while continueRoutine:
                ...
                # *story_num_txt* updates
                
                # if story_num_txt is starting this frame...
                if story_num_txt.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                    story_num_txt.frameNStart = frameN  # exact frame index
                    story_num_txt.tStart = t  # local t and not account for scr refresh
                    story_num_txt.tStartRefresh = tThisFlipGlobal  # on global time
                    win.timeOnFlip(story_num_txt, 'tStartRefresh')  # time at next scr refresh
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'story_num_txt.started')
                    # update status
                    story_num_txt.status = STARTED
                    story_num_txt.setAutoDraw(True)
                
                # if story_num_txt is active this frame...
                if story_num_txt.status == STARTED:
                    # update params
                    pass
            ...
            # --- Ending Routine "present_story_num" ---
            for thisComponent in present_story_numComponents:
                if hasattr(thisComponent, "setAutoDraw"):
                    thisComponent.setAutoDraw(False)
            thisExp.addData('present_story_num.stopped', globalClock.getTime(format='float'))
            # check responses
            ...
            # the Routine "present_story_num" was not non-slip safe, so reset the non-slip timer
            routineTimer.reset()
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('story.started', globalClock.getTime(format='float'))
        story_audio.setSound(file_names, secs=story_len, hamming=True)
        story_audio.setVolume(1.0, log=False)
        story_audio.seek(0)
        # keep track of which components have finished
        storyComponents = [story_audio, text_plus]
        ...
        while continueRoutine:
            ...
            # if story_audio is starting this frame...
            if story_audio.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                story_audio.frameNStart = frameN  # exact frame index
                story_audio.tStart = t  # local t and not account for scr refresh
                story_audio.tStartRefresh = tThisFlipGlobal  # on global time
                # add timestamp to datafile
                thisExp.addData('story_audio.started', tThisFlipGlobal)
                # update status
                story_audio.status = STARTED
                story_audio.play(when=win)  # sync with win flip
            
            # if story_audio is stopping this frame...
            if story_audio.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > story_audio.tStartRefresh + story_len-frameTolerance:
                    # keep track of stop time/frame for later
                    story_audio.tStop = t  # not accounting for scr refresh
                    story_audio.tStopRefresh = tThisFlipGlobal  # on global time
                    story_audio.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'story_audio.stopped')
                    # update status
                    story_audio.status = FINISHED
                    story_audio.stop()
            # update story_audio status according to whether it's playing
            if story_audio.isPlaying:
                story_audio.status = STARTED
            elif story_audio.isFinished:
                story_audio.status = FINISHED
        ...
        for thisComponent in storyComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('story.stopped', globalClock.getTime(format='float'))
        story_audio.pause()  # ensure sound has stopped at end of Routine
        # the Routine "story" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('question.started', globalClock.getTime(format='float'))
        ...
        # keep track of which components have finished
        questionComponents = [question_text, question_resp]
        ...
        for thisComponent in questionComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('question.stopped', globalClock.getTime(format='float'))
        # check responses
